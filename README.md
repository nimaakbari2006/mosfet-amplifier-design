...
Design process
Architecture selection — cascaded common-source stages for gain, source follower for load isolation and drive strength
Hand calculations — device sizing and biasing solved from MOSFET square-law equations (ID ≈ 41.83 µA per CS stage; buffer bias solved via the source-follower quadratic, ID4 = 158 µA)
Schematic capture in KiCad
SPICE simulation using process BSIM4 NMOS models — DC operating point, AC (loaded and unloaded), and transient analyses, with iterative refinement until all specs were satisfied
